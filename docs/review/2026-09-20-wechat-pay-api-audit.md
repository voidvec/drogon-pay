...
- 同文件其余 `clearReservation(..., [](bool){})` 的 fire-and-forget **与本轮
  修的缺陷同形且同样可达**，建单之前的六处形状拒绝全在此列：`:1413` 币种非法、
  `:1432` `time_expire` 非法、`:1448` 微信金额非法、`:1462` 微信字段越界、
  `:1484` `notify_url` 未过 SSRF 门、`:1518` 渠道不存在。六处都在
  `checkAndSetStatus`（`:1350`）取到预留**之后**应答，客户端按 400 更正参数
  立即重试时，删除尚未落地，重试会撞上"1004 idempotency request in
  progress"——被一笔已经失败的请求拒在门外。
  本轮**未动**它们，原因不是判断无碍，而是用户把本轮边界划在"CI 与评审意见
  修完即停"：改动范围不含新增测例。列为**下一轮首位候选**（修法与本轮同形，
  即把应答接进删除回调；六处的 transport 文案与 body 文案两两不同形——
  例如 `:1413` 的 `"invalid currency"` 与 `:1448` 的
